...
def create_obj(bucket_name,obj_name):
    s3 = boto3.resource('s3')
    s3.Object(bucket_name,obj_name).put(Body='')
def LandingPageBackground(bucket_name):
    ...
    s3 = boto3.client('s3')
    try:
        s3.put_object(Body=image_bytes.getvalue(), Bucket='6degreesimages',Key='LandingPage')
    except Exception:
        print("Error placing url in s3")
    return
...
def lambda_handler(event,context):
    LandingPageBackground('6degreesimages')
    try:
